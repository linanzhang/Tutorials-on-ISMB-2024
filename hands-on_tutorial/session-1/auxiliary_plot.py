...
def plot_spatial_activity(adata, genes, points, edges):
    ...
    ngenes = len(genes)
    fig, axs = plt.subplots(1, ngenes, figsize=(ngenes*figsize, figsize), dpi=100)
...
def plot_umap(adata, palette=None, is_tf=True):
    ...
    fig, axs = plt.subplots(1,2, figsize=(figsize*2.25, figsize), width_ratios=[1.25,1], dpi=100)
...
def plot_heatmap(df_ct_tf, tf_list, ct_list, clip=10):
    ...
    figsize = 0.25
    plt.figure(figsize=(figsize*len(x_lab)*0.9, figsize*len(y_lab)), dpi=100)
